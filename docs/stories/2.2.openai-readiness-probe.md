...
- [x] Extend `src/cli/diagnostics.py` (or dedicated probe module) with an `openai-probe` subcommand that loads `OpenAISettings`, performs chat + embedding calls, writes sanitized JSON output under `artifacts/openai/probe.json`, and returns exit code 0 on success. (AC: 1) [Source: docs/architecture.md#cli-orchestrator] [Source: docs/architecture/source-tree.md#source-tree-blueprint]
  ...
  - [x] Reuse diagnostics redaction utilities (e.g., `src/cli/diagnostics_sanitizer.py`) to scrub artifacts/logs and document the shared helper for future probes. (AC: 1,2) [Source: docs/stories/2.1.openai-model-guardrails.md#dev-notes] [Source: docs/architecture/coding-standards.md#critical-rules]
...
### Secrets Handling
- Reports and logs must mask API keys and prompt contents, reusing sanitization routines from diagnostics (`src/cli/diagnostics_sanitizer.py`) to maintain compliance and parity across probes. [Source: docs/architecture/coding-standards.md#critical-rules]
...
### Open Questions
- Pending Product Owner confirmation on whether probe should optionally skip live chat calls in air-gapped CI environments.
