...
## Goal
Route all chat-based LLM calls through Codex CLI (gpt-5.1-mini, medium reasoning) while leaving embeddings on the existing local embedding model/dimensions.

## Architecture
Add a Codex exec client + LLM adapter that implements neo4j-graphrag's `LLMInterface`, then switch LLM provider selection via config while keeping embeddings on `SharedOpenAIClient`. Persist Codex JSONL logs/last messages under the ingestion QA report directory for debugging.

## Tech Stack
Python 3.12, neo4j-graphrag `LLMInterface`, Codex CLI (`codex exec`), Ruff, pytest.
